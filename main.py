...
def check_modules(required_modules):
    for module in required_modules:
        try:
            __import__(module)
            print(Fore.RED + f"[ERROR] | {module} is already installed.")
        except ImportError:
            return False
    return True
...
def install_modules():
    ...
    if check_modules(required_modules):
        print(Fore.RED + "[ERROR] | You already installed all modules!")
        return

    print(Fore.YELLOW + "[INFO] | Installing modules...")
    for module in required_modules:
        os.system(f'pip install {module}')
        print(Fore.GREEN + f"[INFO] | Installed module: {module}")
...
time.sleep(5)
print(Fore.YELLOW + "[CHECK] | Checking webhook...")
time.sleep(2.9)
print(Fore.YELLOW + "[INFO] | Starting bot...")
time.sleep(5)
os.system('cls' if os.name == 'nt' else 'clear')
...
async def username(session, user_id):
    while True:
        ...
        if "name" in response:
            ...
            print(Fore.YELLOW + f"[ERROR] | Ratelimited | User ID: {user_id}")
        else:
            print(Fore.RED + f"[ERROR] | Username not found | User ID: {user_id}")
            return None
        ...
        if "created" in response:
            ...
            for format_str in datetime_formats:
                try:
                    ...
                    continue
            print(Fore.RED + f"[ERROR] | Unable to get creation date | User ID: {user_id}")
            return None
        elif "status" in response and response["status"] == 429:
            await asyncio.sleep(1)
            print(Fore.YELLOW + f"[ERROR] | Ratelimited | User ID: {user_id}")
    ...
    for asset_id in asset_ids:
        while True:
            ...
            if "data" in response and response["data"]:
                ...
                await asyncio.sleep(1)
                Logger.warning(Fore.YELLOW + f"[ERROR] | Ratelimited | User ID: {user_id}")
            else:
                break
                ...
                if last_online:
                    ...
                    for format_str in datetime_formats:
                        try:
                            ...
                            continue
            print(Fore.RED + f"[ERROR] | Unable to get last online date | User ID: {user_id}")
            return None
